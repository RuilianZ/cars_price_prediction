...
For analysis completeness, we dropped NA values. For better illustration, we transformed the original unit (100000 Indian Rupee) of response variable **price** to USD, using the exchange rate of 100000 Indian Rupee equals to 1309.75 USD on March 21, 2022. For numeric variables with unit, the units are deleted for model-building. We also delete the car model from the original `name` variable, keeping only the brand name, since the model can be greatly explained by the predictors we are using. The type `Fourth` in owner_type is actually Fourth and above.
...
First, we look at the price distribution for all 5 categorical predictors respectively. There are some interesting findings:  

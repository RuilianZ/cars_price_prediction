...
```
F
rom the variance importance plots above, `power` and `year` exist in all four plots, indicating that they are important predictors.
...
as.data.frame(cbind(model, test_MSE)) %>% 
  knitr::kable()
```

For the 3 linear models, the test error is calculated using the test data. The PLS model has the least test error, so it is more preferred among the linear models.
